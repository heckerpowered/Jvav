...
void SourceText::ParseLines() noexcept
{
    ...
    while (Position < Text.length())
    {
        ...
        {
            AddLine(Position, LineStart, LineBreakWidth);
...
void SourceText::AddLine(const std::size_t Position, const std::size_t LineStart, const std::size_t LineBreakWidth) noexcept
{
    const auto LineLength = Position - LineStart;
    const auto LineLengthIncludingLineBreak = LineLength + LineBreakWidth;
    PrivateLines.emplace_back(LineStart, LineLength, LineLengthIncludingLineBreak);
}
